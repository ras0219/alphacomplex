...
namespace ai {
    ...
    struct AI : ecs::ComponentCRTP<ecs::Component::AI, AI> {
        using priority_t = int;
        using script_ptr = std::shared_ptr<AIScript>;
        using timer_t = int;
    ...
    struct AISystem : ecs::SubSystem<AISystem, AI> {
        void update_item(ecs::Ent*, AI* ai);

        static ecs::CRTPSystemFactory<AISystem> factory;
    };

    /// Interface to be implemented by AI script coroutines
    struct AIScript {
        /// Convenience alias for the AI::timer_t type
        ///@see AI::timer_t
        using timer_t = AI::timer_t;
        ...
        /// Default behavior: Complete the current script.
